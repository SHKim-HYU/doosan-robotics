...
void OnDisConnected() {
  ...
  while (!Drfl.open_connection("192.168.12.249")) {
...
int main(int argc, char** argv) {
  ...
  assert(Drfl.open_connection("192.168.12.249"));
      ...
      case '1':
          {
              ...
              Drfl.connect_rt_control("192.168.12.249",12347);
      ...
      case 't': // torque
        ...
        memcpy(buf_tor_ext, Drfl.read_data_rt()->external_joint_torque, sizeof(float)*NUMBER_OF_JOINT);
          ...
          for (int i=0; i<6; i++) {
              ...
              trq_d_tmp[i] = trq_g[i]+kp[i]*err_q[i]+kd[i]*err_q_dot[i]+tor_ext[i];
              // trq_d_tmp[i] = trq_g[i];
              // trq_d[i]=trq_g[i];
          ...
          cout <<"Robot Mode : " << Drfl.read_data_rt()->robot_mode << ", Robot State : " << Drfl.read_data_rt()->robot_state << ", Control Mode : " << Drfl.read_data_rt()->control_mode<<endl;
